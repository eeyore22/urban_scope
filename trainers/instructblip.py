...
class InstructBLIPTrainer(BaseTrainer):
    def build_processor(self):
        return InstructBlipProcessor.from_pretrained(self.args.model)
    ...
    def build_model(self):
        ...
        if self.args.lora:
            ...
            # d) freeze LM input embeddings to avoid in-place autograd errors
            for p in model.language_model.get_input_embeddings().parameters():
                p.requires_grad_(False)

            # e) hook to cast to the right dtype
            mp_dtype = (
            ...
            def _detach_and_cast(_, __, out):
                return out.detach().to(mp_dtype) # cut autograd + match dtype 
            
            model.language_model.get_input_embeddings() \
                 .register_forward_hook(_detach_and_cast)

            # f) patch the forward to drop duplicated inputs_embeds that PEFT sometimes injects
            def _peft_forward(self, *args, **kwargs):
                if len(args) and "inputs_embeds" in kwargs:
                    kwargs.pop("inputs_embeds")
                return self.base_model(*args, **kwargs)
            
            model.forward = MethodType(_peft_forward, model)

        # Full-precision / mixed-precision path
        else:
            dtype = (
                torch.bfloat16 if self.args.bf16
                else torch.float16  if self.args.fp16
        ...
        # optional vision freezing
        if self.args.freeze_vision:
            for name, param in model.named_parameters():
                if "vision_tower" in name or "qformer" in name:
    ...
    def evaluate(self):
        ...
        evaluate_model(
            ...
            adapter=adapter,
            processor_fn=lambda d: InstructBlipProcessor.from_pretrained(d),
            model_fn=lambda d: InstructBlipForConditionalGeneration.from_pretrained(d),
...
def get_args():
    ...
    parser.add_argument("--out_dir", type=str, default="outputs/instructblip")
...
if __name__ == "__main__":
    ...
    if args.train_json and args.val_json: 
        # Fine-tune mode
        trainer = InstructBLIPTrainer(args)
        trainer.train()
    else: 
